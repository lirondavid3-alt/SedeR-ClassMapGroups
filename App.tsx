...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const testConnection = async () => {
            if (!db) return;
            try {
                const { testFirebaseConnection } = await import('./services/firebase');
                const result = await testFirebaseConnection();
                if (!result.success) {
                    if (result.code === 'auth/api-key-not-valid') {
                        toast.error("מפתח ה-API של Firebase אינו תקין. ייתכן שיש הגבלה ב-Google Console.");
                    } else if (result.code === 'auth/unauthorized-domain') {
                        toast.error("הדומיין הנוכחי אינו מורשה ב-Firebase Console.");
                    }
                }
    ...
    switch (currentScreen) {
            case 'login':
                return <LoginScreen onLogin={() => {}} onGuestMode={handleGuestLogin} />;
            case 'main':
    ...
    if (currentScreen === 'login' && !currentUser) {
        return <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">{renderContent()}</div>;
    }
