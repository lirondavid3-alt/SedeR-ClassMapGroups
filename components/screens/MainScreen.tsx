...
const MainScreen: React.FC<MainScreenProps> = ({ user, profile, allCharts, onStartNew, onLoadChart, onDeleteChart, onDuplicateChart, onDeleteClass, onUpdateClassName, onUpdateChartName, onImportCharts, onReorderCharts, isCloudSync }) => {
    ...
    useEffect(() => {
        if (!user.email || !isFirebaseAvailable || !db) return;
    ...
    return (
        <div className="w-full max-w-4xl mx-auto text-center p-4 md:p-8">
            ...
            {!isFirebaseAvailable ? (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-6 md:mb-8 flex items-center gap-3 text-rose-700 shadow-sm animate-pulse max-w-3xl mx-auto text-right">
                    <AlertCircle className="h-6 w-6 flex-shrink-0" />
                    <div className="text-right">
                        <p className="font-bold text-base md:text-lg">האפליקציה פועלת במצב מקומי בלבד</p>
                        <p className="text-sm opacity-80">שירותי הענן אינם זמינים (ייתכן שמפתח ה-API אינו תקין). כל השינויים נשמרים בדפדפן שלך בלבד ולא יסונכרנו למכשירים אחרים.</p>
                    </div>
                </div>
            ) : isCloudSync ? (
