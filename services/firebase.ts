...
try {
    console.log("--- [DEBUG] SedeR SeatingGroups - Initializing Firebase ---");
    ...
    if (!firebaseConfig.apiKey || firebaseConfig.apiKey.includes('INSERT_API_KEY')) {
        console.warn("Firebase API Key is missing or invalid placeholder.");
...
export { auth, db, analytics, googleProvider, isFirebaseAvailable };

// --- Helper Functions ---

// Test connection and log API Key status (Safe for Debug)
export const testFirebaseConnection = async () => {
    if (!isFirebaseAvailable) return { success: false, error: "Firebase not initialized", code: "initialization-failed" };
    try {
        console.log("--- Firebase Connection Health Check ---");
        console.log("Project ID:", firebaseConfig.projectId);
        console.log("Auth Domain:", firebaseConfig.authDomain);
        // Don't log full API key for safety, but check its presence
        console.log("API Key present:", !!firebaseConfig.apiKey, "Length:", firebaseConfig.apiKey?.length);
        
        // This is a minimal read to verify API Key + Setup
        const dummyRef = doc(db, "_internal", "connection_test");
        await getDoc(dummyRef);
        console.log("Firestore: Connection reached (even if doc doesn't exist)");
        return { success: true };
    } catch (error: any) {
        console.error("Firebase Connection Error Info:", {
            code: error.code,
