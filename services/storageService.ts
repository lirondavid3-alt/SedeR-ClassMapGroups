...
const STORAGE_KEYS = {
    CHARTS: 'seder_charts',
    PROFILE: 'seder_profile',
    EMAIL: 'seder_last_email'
};

const getLocalCharts = (): Chart[] => {
    try {
...
const saveLocalCharts = (charts: Chart[]) => {
...
export const deleteChart = async (chartId: string): Promise<void> => {
    ...
    if (!isFirebaseAvailable || !db) {
        ...
        saveLocalCharts(charts.filter(c => c.id !== chartId));
        return;
    }
...
export const deleteChartsBatch = async (chartIds: string[]): Promise<void> => {
    ...
    if (!isFirebaseAvailable || !db) {
        const charts = getLocalCharts();
        saveLocalCharts(charts.filter(c => !chartIds.includes(c.id)));
        return;
    }
...
export const updateUserAdminFields = async (
    ...
    if (!isFirebaseAvailable || !db) {
        ...
        if (profile) {
            saveLocalProfile({ ...profile, ...updates } as any);
        }
        return;
    }
...
export const loadChartById = async (chartId: string): Promise<Chart | null> => {
    ...
    if (!isFirebaseAvailable || !db) {
        return getLocalCharts().find(c => c.id === chartId) || null;
    }
